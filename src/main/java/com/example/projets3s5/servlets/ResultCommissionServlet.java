...
@WebServlet("/ResultCommissionServlet")
public class ResultCommissionServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if ("research".equalsIgnoreCase(mode)) {
            try (Connection connection = ConnexionPost.getConnectionPost()) {
                ...
                Map<String,Double> commission = Vente.getCommission(connection,ventesDate);

                request.setAttribute("ventes", ventesDate);
                request.setAttribute("commission", commission);


                request.getRequestDispatcher("resultCommission.jsp").forward(request, response);
            } catch (Exception e) {
                e.printStackTrace();
